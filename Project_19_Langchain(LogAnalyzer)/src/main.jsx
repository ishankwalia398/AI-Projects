...
const API_BASE = import.meta.env.DEV ? '' : 'https://log-analyzer-lc-api.vercel.app'
...
function App() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/health`).then(r => r.json()).then(data => setConfigured(Boolean(data.langchain_configured))).catch(() => setConfigured(null))
  }, [])
  ...
  return <div className="app" data-theme={theme}>
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><div className="status-pulse"/><div><strong>{configured === false ? 'AI setup required' : configured ? 'Agents Ready' : 'LangChain log analysis'}</strong><span>Parser · Analyst · Remediation</span></div></div>
    </aside>
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {configured === false && <p className="analysis-note">The API server needs a Command Code or Groq key before LangChain can analyze files.</p>}
