...
def _ask_target(spec, chatbot: ChatbotClient, text: str, rag=None) -> str:
    """Route the question to whichever app this metric is pointed at."""
    if spec.target == "rag":
        return (rag or _RAG).ask(text).answer
    return chatbot.chat(text).reply


def _rag_query(question: str, top_k: int = 3, rag=None) -> dict:
    reply = (rag or _RAG).ask(question, top_k=top_k)
    return {
        "answer": reply.answer,
        "retrieval_context": reply.retrieval_context,
    }
...
def _one_case(spec: MetricSpec, metric, chatbot: ChatbotClient, item, rag=None) -> dict:
    ...
    if spec.kind == "conversational":
        ...
        payload = _rag_query(item.input, rag=rag)
        tc = spec.build_case(item, payload)
        label = item.input
        actual = payload["answer"]
    else:
        label = prompt_of(item)
        actual = _ask_target(spec, chatbot, label, rag=rag)
...
def run_spec(
    ...
    try:
        ...
        for item in window:
            rows.append(_one_case(spec, metric, chatbot, item, rag=rag))
