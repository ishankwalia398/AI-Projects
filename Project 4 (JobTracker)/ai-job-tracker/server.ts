...
// Helper to query Grok or Gemini
async function getAICompletion(prompt: string, systemInstruction?: string, jsonSchema?: any): Promise<string> {
  const xaiKey = process.env.XAI_API_KEY;

  if (xaiKey) {
    // Attempt Grok API call (OpenAI-compatible)
    try {
      const messages: any[] = [];
      ...
      const requestBody: any = {
        model: 'grok-beta', // or standard grok-2 / grok-vision-preview
        messages,
        temperature: 0.7,
      };
...
async function startServer() {
  ...
  // Endpoints: AI Cover Letter Creator & Outreach Generator
  ...
  // Endpoints: AI Resume-JD Match Score
  app.post("/api/ai/match-score", async (req, res) => {
    ...
    const matchSchema = {
      ...
      properties: {
        ...
        executiveSummary: { type: Type.STRING, description: "A high-level summary of the overall alignment and level of fit" }
      },
  ...
  // Endpoints: AI Dashboard / Pipeline Insights
  app.post("/api/ai/insights", async (req, res) => {
    ...
    // Format mini applications payload to conserve tokens and focus on structure
    const appPayload = applications.map((app: any) => ({
      company: app.company,
      role: app.role,
      status: app.status,
    ...
    const insightSchema = {
      type: Type.OBJECT,
      properties: {
        summary: { type: Type.STRING, description: "High-level review of the user's current search velocity, diversity of roles, and active pipeline health" },
        strengths: {
          type: Type.ARRAY,
          items: { type: Type.STRING },
  ...
  // Integration of Vite middleware for serving state in Dev vs Production
