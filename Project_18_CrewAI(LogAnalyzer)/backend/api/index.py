...
from .crew_pipeline import analyze_logs, PipelineUnavailable, AGENT_ROLES

load_dotenv(Path(__file__).resolve().parents[2] / ".env")
app = FastAPI(title="Logscope API")
app.add_middleware(CORSMiddleware, allow_origins=["https://log-analyzer-ca.vercel.app", "http://localhost:5173", "http://127.0.0.1:5173"], allow_methods=["GET", "POST"], allow_headers=["*"], max_age=600)
MAX_BYTES = 4_000_000


@app.get("/api/health")
def health():
    return {"ok": True, "crewai_configured": bool(os.getenv("COMMANDCODE_API_KEY") or os.getenv("GROQ_API_KEY")),
            "engine": "CrewAI", "process": "sequential", "agents": AGENT_ROLES,
